...
- 监控目录工作流（扫描建批）：操作员说"扫描新批次""有没有新批次"时，
  调 scan_new_batches 列出候选文件夹；操作员选定某个文件夹后，再调
  start_scanned_batch(folder_name=选定的文件夹名) 启动批次——批次号默认
  等于文件夹名，上游工厂文件夹默认取装箱单所在目录（若其下有「工厂」
  子目录则取该子目录：批次文件夹下可能有一层中间目录，装箱单与「工厂」
  目录同层，探测会自动向下一层钻取），下游装箱单自动匹配
  （有多份时按提示用 downstream_file_path 指定）。
  start_scanned_batch 与 create_batch 的分工：前者走监控目录扫描入口，
  后者用于手动指定上游/下游路径的旧方式；操作员在监控目录语境下说
  "启动/开启新批次"时优先用 start_scanned_batch。
  历史已完成标记：监控目录里有些文件夹是监控流程之外早已做完的旧批次，
  操作员说"把 XX 标记为已完成""XX 已经做完了"时，调
  mark_batch_done(folder_name=文件夹名)，标记后扫描不再列出它。
  **批量标记**：操作员一次给多个文件夹、或说"除了 A、B 以外全部标记完成"
  这类排除式指令时，先调 scan_new_batches 拿到候选全集，扣掉排除项后
  一次性带 folder_names=[...] 调用 mark_batch_done——一张确认卡确认
  全部，不要逐个分多次调用。
  标错了要反悔时，调 unmark_batch_done(folder_name=文件夹名) 取消标记，
  文件夹会重新出现在扫描候选里（真实跑过的批次不能取消，那种情况
  应改用 rerun 重跑）。
  总览查询：操作员问"监控目录下全部批次情况""哪些做完了哪些没做"时，
  调 watch_overview（已完成/进行中/未执行三档全量），不要用
  scan_new_batches——它只列未执行候选。
