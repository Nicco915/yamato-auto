...
- handle_message：对话主入口（确认前），先走 triage.run_triage 结构化分诊，
  ...
  - intent == "qa"：操作指南类问题，直接走 guide.ask_guide 直答（清槽位），
    不进 loop；
  - intent == "clarify"：反问补齐——已提取参数以槽位形式暂存 session，
    多轮凑齐；target_tool 为空且已有槽位视为用户中止，清槽位；
  - intent == "action" 前槽位无条件合并落槽（target_tool 变化则旧槽位
    作废），再按置信度三级路由（代码侧映射，不动 TriageResult 契约）：
    黄灯（0.6–0.8 且目标是写工具）**先于缺参判定**存 soft_pending 软
    挂起 + 回复确认式反问，不进 loop——下一条消息"是"则按已存意图
    直接带 hint 进 loop（不再过 triage；提升前复检必填槽位，仍缺则
    代码反问缺失项、槽位保留等下轮补齐），"算了"则全清，其他消息
    清挂起后正常走新一轮分诊；
  - 缺参：一律代码生成缺参反问（不播模型的 yes/no 式文案——缺参时
    它是死胡同），槽位已落 session，等下轮补齐合并；
  - 绿灯（>0.8 且参数齐）：槽位已落 session，带 triage_hint 进
    loop.run_dispatch；黄灯豁免（0.6–0.8 但只读工具）与红灯（<0.6）
    不带 hint 走旧循环；
...
def _consume_soft_pending(session: _sessions.DispatcherSession, message: str,
    ...
    - 确认（是/对/好的/ok…）：按已存 target_tool+slots 提升为高置信
      action（带 hint 进 loop），不再过 triage；提升前复检必填槽位，
      仍缺（布防时槽位未凑齐）则不进 loop 烧轮次，代码直接反问缺失项，
      槽位保留等下轮补齐；
    ...
    if text in _SOFT_CONFIRM_YES:
        ...
        # 缺参复检：布防时槽位未凑齐的场景（黄灯分支允许带缺参布防），
        # 确认后仍缺——不进 loop 烧轮次，代码直接反问缺失项，槽位保留
        missing = _triage.missing_required(tool, slots)
        if missing:
            names = "、".join(_PARAM_CN.get(p, "必要信息") for p in missing)
            reply = f"好的，请提供{names}后再继续。"
            _sessions.record_turn(session, message, reply)
            if on_progress:
                on_progress({"type": "final", "message": reply})
            return {"status": "ok", "message": reply, "intent": "clarify"}
        # 提升为高置信 action：带 hint 进 loop（确认门语义不变——
        # 写工具仍会出预览卡等人工确认）
...
def handle_message(message: str, session_id: str | None = None, *, phase: int = 2,
    ...
    if triage_result is None:
        ...
        # 槽位无条件合并落槽：target_tool 变化则旧槽位作废（merge_slots
        # 语义），后续各分支只读 merged，不再各自 set_slots
        ...
        if triage_result.intent == "clarify":
            # 模型主动反问/中止：target_tool 为空且已有槽位 = 用户中止，
            # 清槽位（先判再清——clear_slots 会把 current_slots 清空）
            if triage_result.target_tool is None and session.current_slots:
                _sessions.clear_slots(session)
            reply = triage_result.reply_message
            if not reply:
                names = "、".join(_PARAM_CN.get(p, "必要信息") for p in missing)
                reply = f"请提供{names}后再继续。" if names else "请补充说明您的需求。"
            _sessions.record_turn(session, message, reply)
            if on_progress:
                on_progress({"type": "final", "message": reply})
            result = {"status": "ok", "message": reply, "intent": "clarify"}
        elif (0.6 <= triage_result.confidence <= 0.8
              and triage_result.target_tool in _WRITE_TOOL_NAMES):
            # 黄灯区（提前到缺参判定之前）：0.6–0.8 置信的写工具请求——
            # 不让模型猜测的参数直接冲进确认卡，先存软挂起 + 回复确认式
            # 反问，等操作员一轮内确认（下一条消息由入口短路消费）；
            # 即使必填槽位未凑齐也布防，「是的」由入口短路消费时再做
            # 缺参复检（见 _consume_soft_pending），不再让缺参抢跑短路
            # 软挂起；只读工具查错无代价，不拦截（落入下方旧循环）
            ...
            # 缺参：模型的 reply_message 可能是 yes/no 式确认反问——缺参时
            # 它是死胡同（用户答「是」也凑不齐参数），一律用代码生成的缺参
            # 反问；槽位已落 session，等下轮补齐合并
            names = "、".join(_PARAM_CN.get(p, "必要信息") for p in missing)
            ...
            # 绿灯（高置信 action）：槽位已落 session，带 hint 进 loop
