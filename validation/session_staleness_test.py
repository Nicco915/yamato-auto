# -*- coding: utf-8 -*-
"""会话缓存新鲜度校验测试（方案C兜底，2026-08-05；2026-08-10 适配每批次独立缓存）。

背景：会话缓存落在 data/sessions/{safe(batch_id)}/{工厂名}.json，按批次号
隔离目录。第一批次残留的 JSON 里 targets[].path / items[].source_file 全是
上一批次路径，第二批次（upstream_root 可能已换目录）启动时 Node3 直接命中
旧缓存，导致全厂误用上一批次数据。方案C 在缓存命中前校验路径证据新鲜度。
...
8. 跨批次同 upstream_root，session 应失忆：批次 A 的缓存不在批次 B 目录下
   → 自然返回 None（每批次独立目录的天然隔离）。
...
def _write_cache(batch_id: str, factory: str, data: dict) -> Path:
    """写假 session 到 data/sessions/{safe(batch_id)}/{factory}.json。

    通过 app.extraction.session.batch_session_path 取真实生产路径，
    与 service._ensure_batch_session_dir 一致的目录布局。
    """
    from app.extraction.session import batch_session_path
    path = batch_session_path(batch_id, factory)
...
def case_1_fresh_hit() -> None:
    ...
    got = extraction_node._try_load_cached_session("批次-1", "厂1", str(NEW_ROOT))
...
def case_2_stale_old_root() -> None:
    ...
    _write_cache("批次-2", "厂2", {
        "items": {"S1": _item(str(OLD_FILE))},
        "no_code_items": [],
        "targets": [{"path": str(OLD_FILE), "barcodes": ["4900000000001"]}],
    })
    got = extraction_node._try_load_cached_session("批次-2", "厂2", str(NEW_ROOT))
...
def case_3_no_evidence() -> None:
    ...
    _write_cache("批次-3", "厂3", {
        "items": {"S1": _item("extraction_error: boom")},
        "no_code_items": [],
        "targets": [],
    })
    got = extraction_node._try_load_cached_session("批次-3", "厂3", str(NEW_ROOT))
...
def case_4_none_upstream_legacy() -> None:
    ...
    _write_cache("批次-4", "厂4", {
        "items": {"S1": _item(str(OLD_FILE))},
        "no_code_items": [],
        "targets": [{"path": str(OLD_FILE), "barcodes": []}],
    })
    got = extraction_node._try_load_cached_session("批次-4", "厂4")
    assert got is not None, "upstream_root=None 应保持老行为直接命中"
    got2 = extraction_node._try_load_cached_session("批次-4", "厂4", None)
...
def case_6_under_root_but_deleted() -> None:
    ...
    _write_cache("批次-6", "厂6", {
        "items": {"S1": _item(str(ghost))},
        "no_code_items": [],
        "targets": [{"path": str(ghost), "barcodes": []}],
    })
    got = extraction_node._try_load_cached_session("批次-6", "厂6", str(NEW_ROOT))
...
def case_7_evidence_priority() -> None:
    ...
    _write_cache("批次-7a", "厂7a", {
        "items": {"S1": _item(str(OLD_FILE))},
        "no_code_items": [],
        "targets": [{"path": str(NEW_FILE), "barcodes": []}],
    })
    got = extraction_node._try_load_cached_session(
        "批次-7a", "厂7a", str(NEW_ROOT))
    assert got is not None, "targets[].path 新鲜即命中，不看 source_file"

    # 7b. targets 为空，source_file 在新 root 下且存在 → 回落 source_file 命中
    _write_cache("批次-7b", "厂7b", {
        "items": {"S1": _item(str(NEW_FILE))},
        "no_code_items": [],
        "targets": [],
    })
    got = extraction_node._try_load_cached_session(
        "批次-7b", "厂7b", str(NEW_ROOT))
    assert got is not None, "targets 为空应回落 source_file 判定"

    # 7c. targets 为空，no_code_items 的 source_file 在新 root 下 → 也作证据
    _write_cache("批次-7c", "厂7c", {
        "items": {},
        "no_code_items": [{"sku_code": "", "source_file": str(NEW_FILE)}],
        "targets": [],
    })
    got = extraction_node._try_load_cached_session(
        "批次-7c", "厂7c", str(NEW_ROOT))
    assert got is not None, "no_code_items[*].source_file 也应作为回落证据"
    print("  ✓ 证据优先级：targets[].path 优先，空时回落 items/no_code_items 的 "
          "source_file")


def case_8_cross_batch_isolation() -> None:
    """跨批次同 upstream_root：批次 A 的缓存不在批次 B 目录下 → 自然 None。

    每批次独立 session 目录：data/sessions/{safe(A)}/{factory}.json 与
    data/sessions/{safe(B)}/{factory}.json 物理隔离。批次 B 调
    _try_load_cached_session(batch_id=B, ...) 自然找不到批次 A 的缓存。
    """
    # 批次 A 写一份新鲜缓存（同 NEW_ROOT 下）
    _write_cache("批次-A", "厂甲", {
        "items": {"S1": _item(str(NEW_FILE))},
        "no_code_items": [],
        "targets": [{"path": str(NEW_FILE), "barcodes": ["4900000000001"]}],
    })
    # 批次 B 用同 upstream_root、同时给新批次 id → 应返回 None
    got = extraction_node._try_load_cached_session(
        "批次-B", "厂甲", str(NEW_ROOT))
    assert got is None, f"跨批次应失忆：批次 B 目录下无 session.json → None，got={got}"
    # 批次 A 自己的查询仍命中（隔离不破坏同批次命中）
    got_a = extraction_node._try_load_cached_session(
        "批次-A", "厂甲", str(NEW_ROOT))
    assert got_a is not None, "批次 A 自己仍应命中缓存"
    print("  ✓ 跨批次同 upstream_root：批次 B 失忆，批次 A 自己仍命中")
...
CASES = [
    ...
    ("8. 跨批次同 upstream_root → 失忆（独立目录）", case_8_cross_batch_isolation),
]
